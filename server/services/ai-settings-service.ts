...
export class AiSettingsService {
  ...
  public async getFinalPromptForScenario(agencyId: number, scenarioKey: string): Promise<string> {
    ...
    // إعداد البرومبت الأساسي
    let finalPrompt = settings.finalPrompt;
    
    // إضافة توجيهات اللهجة المصرية إذا كان مطلوبًا
    if (settings.speakEgyptian) {
      finalPrompt += `\n\nمهم جدًا: يجب أن تتحدث باللهجة المصرية بشكل طبيعي وعفوي في ردودك. استخدم مفردات وتعبيرات مصرية حقيقية وليست مجرد ترجمة حرفية. تحدث كما يتحدث المصريون في المحادثات اليومية. تجنب استخدام اللغة الفصحى تمامًا إلا في الاقتباسات الرسمية.`;
    }
    
    // إضافة توجيهات المصطلحات التقنية إذا كان مطلوبًا
    if (settings.useTechnicalTerms) {
      finalPrompt += `\n\nعند الحديث عن مواضيع تقنية، استخدم المصطلحات التقنية الصحيحة باللغة الإنجليزية إذا لزم الأمر. يمكنك استخدام مصطلحات مثل SEO، CRM، UI/UX، API، conversion rate وغيرها من المصطلحات المتخصصة في مجال التسويق الرقمي وتطوير المواقع وإدارة المشاريع.`;
    }
...
مهم: قدم إجابات موجزة وتفاعلية. لا تطرح أكثر من سؤال واحد في كل رد. استخدم أسلوب محادثة طبيعي. لا تكرر نفس الأسئلة أو المعلومات.`;
    }
...
اقترح عناوين جذابة، وأفكار محتوى أصلية، واستخدم أساليب سرد مناسبة للجمهور المستهدف.`;
        break;
      case 'marketing-strategy':
        finalPrompt += `
\n\nأنت الآن في سيناريو استراتيجية التسويق. ساعد العميل في تطوير استراتيجية تسويق فعالة للمشروع.
اقترح قنوات تسويق مناسبة، واستراتيجيات محتوى، وطرق لزيادة التحويلات وتحسين معدلات التفاعل.`;
        break;
      case 'chat':
        finalPrompt += `
\n\nأنت في محادثة عامة مع العميل. كن ودودًا ومتعاونًا وقدم مساعدة قيمة.
استجب بشكل طبيعي للأسئلة والاستفسارات، واقترح أفكارًا وحلولًا عندما يكون ذلك مناسبًا.`;
        break;
      default:
        break;
    }
